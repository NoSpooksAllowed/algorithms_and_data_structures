...
fn max_pairwise_product(numbers: &Vec<i32>) -> i64 {
    let mut max_index1 = -1;

    for i in 0..numbers.len() {
        if max_index1 == -1 || numbers[i] > numbers[max_index1 as usize] {
            max_index1 = i as i32;
        }
    }
    ...
    for j in 0..numbers.len() {
        if j as i32 == max_index1 {
            continue;
        }

        if max_index2 == -1 || numbers[j] > numbers[max_index2 as usize] {
            max_index2 = j as i32
        }
    }
    numbers[max_index1 as usize] as i64 * numbers[max_index2 as usize] as i64
}
...
/* Because grader does not allow to include add modules, I have to explicit copy and paste this
 * code */
